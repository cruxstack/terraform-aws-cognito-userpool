locals {
  ...
  sms_role_arn         = local.enabled ? coalesce(try(var.sms_config.sns_caller_arn, ""), aws_iam_role.sms[0].arn) : null
  sms_role_external_id = local.enabled ? coalesce(try(var.sms_config.external_id, ""), random_uuid.sms_role_external_id.result) : null
}
